...
***********************************************************************************************************************************/
// UTF-8 database name as \xNN escapes to keep the source 7-bit ASCII; decodes to the Greek "eureka"
#define UTF8_DB_NAME                                                "\xce\xb5\xcf\x8d\xcf\x81\xce\xb7\xce\xba\xce\xb1"
...
testRun(void)
{
    ...
    if (testBegin("restoreRecoveryWrite*()"))
    {
        ...
        TEST_ERROR(
            restoreRecoveryWriteAutoConf(&fileInfo, PG_VERSION_12, restoreLabel), OptionInvalidError,
        ...
        restoreRecoveryWriteAutoConf(&fileInfo, PG_VERSION_12, restoreLabel);
    ...
    if (testBegin("cmdRestore()"))
    {
        ...
        manifestCipherSpecSet(
            manifestEncrypted, cipherSpecNewP(cipherTypeAes256Cbc, BUFSTRDEF(TEST_CIPHER_PASS_ARCHIVE), .digest = hashTypeSha1));
        ...
        InfoArchive *infoArchive = infoArchiveNew(
            ...
            cipherSpecNewP(cipherTypeAes256Cbc, BUFSTRDEF(TEST_CIPHER_PASS_ARCHIVE), .digest = hashTypeSha1));
        infoArchiveSaveFile(
